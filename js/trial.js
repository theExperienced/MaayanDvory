...
const init = function () {
    ...
    scene.add(particles);
    particles.position.set(0, 0, 0);//copy(city.position);




    gmaterial = new THREE.MeshPhongMaterial({color:0xf4f4f4, side:THREE.BackSide});
    gparticular = new THREE.CircleGeometry(.035, 3);
    aparticular = 35;
    ...
    firmamentGeo = new THREE.SphereBufferGeometry(ballRadius * 22, 70, 20, 0, Math.PI * 2, 0, Math.PI );
    ...
    firmament.position.set(0, 0, -170);
    ...
    maxTower = 2;
    ballRadius = 10;
    towerColors = [0x6a00f4, 0xa100f2, 0xd100d1, 0xf20089];
    mouse = {
        x: 0,
        y: 0
    }
    ...
    for (let i = 0; i < numOfTowers; i++) {
        towerGeo = new THREE.BoxBufferGeometry(.05, .05, .15);//OctahedronBufferGeometry(.5);
        towerMat = new THREE.MeshPhysicalMaterial({
            color: towerColors[Math.floor(Math.random() * towerColors.length)],
            roughness: .5,
            metalness: .35
        });
...
const update = function () {
    ...
    for (let i = 0; i < pointingTowers.length; i++) {
        pointingTowers[i].scale.z = (Math.sin(now * pointingTowersVels[i])  / 2 + .5) * 11 + 1;
        // pointingTowers[i].position.x = Math.cos(now * pointingTowersVels[i]) * ballRadius;
        pointingTowers[i].lookAt(diamondBody.position.x, diamondBody.position.y, diamondBody.position.z);
    }
    ...
    pointLight.position.set(
        Math.sin(now * 2 + Math.PI) * ballRadius * 1.2, 
        Math.cos(now * 1.3 + Math.PI) * ballRadius * 1.2, 
        Math.cos(now * 3 + Math.PI) * ballRadius * 1.2
    );
...
gsap.to(towerBall.position, {
    scrollTrigger: {
        ...
        onUpdate: (i, target) => {
            // console.log('towerball comin to ya', i)
        }
    },
    x: 5,
    y: 5,
...
gsap.to(towerBall.rotation, {
    scrollTrigger: {
        ...
        // toggleActions: 'play none none reverse',
        // onUpdate: (i, target) => {
        //     console.log(diamondBody.position);
        //     for (let i = 0; i < pointingTowers.length; i++) {
        //         pointingTowers[i].scale.z = (Math.sin(now * pointingTowersVels[i])  / 2 + .5) * 11 + 1;
        //         // pointingTowers[i].position.x = Math.cos(now * pointingTowersVels[i]) * ballRadius;
        //         pointingTowers[i].lookAt(diamondBody.position.x, diamondBody.position.y, diamondBody.position.z);
